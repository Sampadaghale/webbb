...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare(
            "INSERT INTO registrationform (UserID, Password, FirstName, Address, Country, Zip_code, Email, Sex, Language, About)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        );
        
        
        $stmt->bind_param("ssssssssss", $userid, $hashed_password, $firstname, $address, $country, $zip, $email, $sex, $lang_string, $about);
    
        if ($stmt->execute()) {
            echo "<h3>Registration Successful!</h3>";
            echo "<p>Welcome, <strong>" . htmlspecialchars($firstname) . "</strong>!</p>";
        } else {
            echo "Error: " . $stmt->error;  // Output any error from MySQL
        }
